...
class AccountServiceImplTest {
    ...
    private val accountService = AccountServiceImpl(accountRepository, userRepository, accountMapper)
    ...
    fun `createAccount - success`() {
        ...
        every { accountRepository.findByAccountNumber("40817810000000000001") } returns null
        every { accountRepository.save(any()) } answers { firstArg() }
        every { accountMapper.toResponse(any()) } returns expected

        val result = accountService.createAccount(1L, "40817810000000000001", AccountType.CHECKING, "RUB")

        assertEquals(expected, result)
        verify { userRepository.findById(1L) }
        verify { accountRepository.findByAccountNumber("40817810000000000001") }
    ...
    fun `createAccount - user not found throws NOT_FOUND`() {
        ...
        val ex = assertThrows<BusinessException> {
            accountService.createAccount(99L, "40817810000000000001", AccountType.CHECKING, "RUB")
        }
    ...
    @Test
    fun `createAccount - duplicate account number throws CONFLICT`() {
        val user = userEntity()
        val existing = accountEntity()

        every { userRepository.findById(1L) } returns Optional.of(user)
        every { accountRepository.findByAccountNumber("40817810000000000001") } returns existing
    ...
    fun `getAccount - success`() {
        ...
        every { accountRepository.findById(1L) } returns Optional.of(entity)
        every { accountMapper.toResponse(entity) } returns expected

        val result = accountService.getAccount(1L)
    ...
    fun `getAccountsByUser - returns list of accounts`() {
        ...
        val entity2 = accountEntity(id = 2L, accountNumber = "40817810000000000002", accountType = AccountType.SAVINGS)
        val response1 = accountResponse(id = 1L)
        val response2 = accountResponse(id = 2L, accountNumber = "40817810000000000002", accountType = "SAVINGS")
